...
import { Candle, IndicatorSnapshot, MarketRegime, SetupType, SignalType, TradePlan } from '../types';
...
export const evaluateSignal = (price: number, indicators: IndicatorSnapshot): DecisionResult => {
  ...
  const longExtended =
    (indicators.atr > 0 && (price - indicators.emaFast) / indicators.atr > 1.8) || indicators.rsi >= 74;
  const shortExtended =
    (indicators.atr > 0 && (indicators.emaFast - price) / indicators.atr > 1.8) || indicators.rsi <= 26;
